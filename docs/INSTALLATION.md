...
conda is useful for heavyweight Python runtimes. uv is useful for fast Python
package and venv workflows. The installer asks for cache/env roots so large
...
user-scoped Miniconda/Miniforge install. Users should not have to activate conda
before running `llm-cli`.
...
Git Bash may expose `CONDA_EXE` and `CONDA_PYTHON_EXE` after `conda init bash`.
Those are emitted by Conda's shell hook, not by `llm-cli`, and they are discovery
hints only. The reliable discovery path is: `CONDA_EXE` if present, `where.exe conda`,
PATH-derived `condabin -> Scripts\conda.exe`, `conda info --base`, registry/uninstall
metadata, then validated PATH fallbacks.

`llm-cli` creates or reuses a managed conda env named `llm-cli-py` and bakes that
env's Python path into wrappers as `VIGYAN_PYTHON`. Base conda Python is only the
bootstrap. If Anaconda default channels require Terms of Service acceptance, the
installer must use explicit paired consent flags such as
`--accept-anaconda-tos --i-accept-anaconda-tos`, not silent acceptance.

Linux follows the same model. The public installer must not hardcode `/vigyan`.
During `llm-cli setup`, ask the user for:

- install mode: standalone or cluster
- install root and package/cache roots
- filesystem profile: auto-detect, ZFS, ext4/xfs, NTFS, or other
- pnpm home and store roots
- conda env root and package cache
- uv cache root
- selected capabilities: PageIndex, telemetry, Linux auditd provenance,
  LiteLLM/OpenCode routing, SSH deployment

Then persist the answers and derive `llm-cli-py` from that config. VVC's
`/vigyan/dev-envs/...` layout is a cluster preset, not the public default.
